...
interface LibraryStore {
    ...
    scanForMusic: (paths?: string[], onProgress?: (current: number, total: number, currentFile?: string) => void) => Promise<void>;
    cancelScan: () => void;
...
export const useLibraryStore = create<LibraryStore>((set, get) => {
    ...
    return {
        ...
        scanForMusic: async (paths?: string[], onProgress?: (current: number, total: number, currentFile?: string) => void) => {
            isScanCancelled = false;
            set({ isScanning: true, scanProgress: 0 });
            try {
                // Use provided paths, or fall back to saved folders, or use defaults
                let scanPaths = paths;
                if (!scanPaths || scanPaths.length === 0) {
                    const settingsState = useSettingsStore.getState();
                    if (settingsState.scanFolders && settingsState.scanFolders.length > 0) {
                        scanPaths = settingsState.scanFolders;
                    } else {
                        scanPaths = fileSystemService.getDefaultMusicPaths();
                    }
                }
                ...
                if (isScanCancelled) {
                    console.log('[LibraryStore] Scan cancelled after filesystem scan');
                    return;
                }
                ...
                set({ scanProgress: 30 });
                onProgress?.(30, 100, 'Found audio files');
                ...
                const profiledTracks = await libraryProfilingService.profileLibrary(
                    ...
                    (current, total) => {
                        ...
                        onProgress?.(current, total, `Profiling track ${current}/${total}`);
                    }
                );

                if (isScanCancelled) {
                    console.log('[LibraryStore] Scan cancelled after profiling');
                    return;
                }
                ...
                set({ scanProgress: 85 });
                onProgress?.(85, 100, 'Merging with existing tracks');
                ...
                if (isScanCancelled) {
                    console.log('[LibraryStore] Scan cancelled before saving');
                    return;
                }

                set({ scanProgress: 90 });
                onProgress?.(90, 100, 'Saving to database');
        ...
        cancelScan: () => {
            ...
            console.log('[LibraryStore] Scan cancellation requested');
        },
